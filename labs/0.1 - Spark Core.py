...
# DBTITLE 1,Task 4.2: Filter by Payment Method
...
from pyspark.sql.functions import col, sum as _sum, countDistinct, avg, count, date_format
...
franchise_metrics_df = (
    ...
    .withColumn("avg_customer_spend", col("total_revenue") / col("unique_customers"))
    # Your implementation here - combine multiple transformations
)

top5_franchises_df = (
    franchise_metrics_df
    .join(franchises_df.select("franchiseID", "name", "city", "country", "size"), "franchiseID", "left")
...
best_day = (
    enriched_txn_df
    .groupBy("day_name", "day_number")
    .agg(_sum("totalPrice").alias("total_revenue"))
    .orderBy(col("total_revenue").desc())
    .limit(1)
)

display(top5_franchises_df)
display(franchise_metrics_df.orderBy(col("total_revenue").desc()))
