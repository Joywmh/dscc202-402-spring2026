...
print(f"Status: {monitor_query.status}")
...
query = (hourly_sales_df
    ...
    .option("checkpointLocation",f"{checkpoint_dir}/hourly_sales_append"  )  # Checkpoint path
...
enriched_stream = (streaming_df
    .join( franchises_df  , on="franchiseID", how="left"      )  # DataFrame to join and join column
    .select(
        col("dateTime"),
        col("franchiseID"),
